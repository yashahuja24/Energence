            ...
            {response && (
              <>
                <div className="mb-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg shadow-sm max-h-60 overflow-y-auto">
                    <ul className="space-y-2">
                      {response.optimized_parameters && Object.entries(response.optimized_parameters).map(([key, value]) => {
                        const questionLabel = questions.find(q => q.id === key)?.question || key.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
                        return (
                          <li key={key} className="flex justify-between">
                            <span className="font-medium text-gray-600">{questionLabel}:</span>
                ...
                <div className="mb-6">
                  ...
                  <div className="p-4 border border-emerald-500 rounded-lg bg-emerald-50 shadow-sm">
                    ...
                    <p className="flex justify-between"><span className="font-medium">Predicted Generation (kW):</span> <span className="text-emerald-700 font-semibold">{response.predicted_generation}</span></p>
                    <p className="flex justify-between"><span className="font-medium">Consumption (kW):</span> <span className="text-emerald-700 font-semibold">{response.consumption}</span></p>
                    <p className="flex justify-between"><span className="font-medium">Power Difference (kW):</span> <span className="text-emerald-700 font-semibold">{response.power_difference}</span></p>
                  </div>
                </div>
              </>
            )}
